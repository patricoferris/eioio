...
CAMLprim value caml_eio_posix_getrandom(value v_ba, value v_off, value v_len) {
  ...
  do {
    void *buf = (uint8_t *)Bytes_val(v_ba) + off;
...
static struct iovec *alloc_iov(value v_bufs) {
  ...
  for (int i = 0; i < n_bufs; i++) {
    ...
    iov[i].iov_base = (uint8_t *)Bytes_val(v_ba) + Long_val(v_off);
...
CAMLprim value caml_eio_posix_pwritev(value v_fd, value v_bufs,
                                      value v_offset) {
  CAMLparam2(v_bufs, v_offset);
  ssize_t r;
  int n_bufs = Wosize_val(v_bufs);
...
CAMLprim value caml_eio_posix_openat(value v_dirfd, value v_pathname,
                                     value v_flags, value v_mode) {
...
static struct custom_operations stat_ops = {
    "eio_posix.stat",           finalize_stat,
    custom_compare_default,     custom_hash_default,
    custom_serialize_default,   custom_deserialize_default,
    custom_compare_ext_default, custom_fixed_length_default};

value caml_eio_posix_make_stat(value v_unit) {
  CAMLparam0();
  CAMLlocal1(v);
  struct stat *data;
  v = caml_alloc_custom_mem(&stat_ops, sizeof(struct stat *),
                            sizeof(struct stat));
  Stat_val(v) = NULL;
...
CAMLprim value caml_eio_posix_fstatat(value v_stat, value v_fd, value v_path,
                                      value v_flags) {
...
intnat ocaml_eio_posix_stat_perm_native(value v_stat) {
  struct stat *s = Stat_val(v_stat);
  return (s->st_mode & ~S_IFMT);
}

value ocaml_eio_posix_stat_perm_bytes(value v_stat) {
  return Val_int(ocaml_eio_posix_stat_perm_native(v_stat));
}

value ocaml_eio_posix_stat_kind(value v_stat) {
  struct stat *s = Stat_val(v_stat);
  return get_file_type_variant(s);
}

int64_t ocaml_eio_posix_stat_rdev_native(value v_stat) {
  struct stat *s = Stat_val(v_stat);
  return s->st_rdev;
}

value ocaml_eio_posix_stat_rdev_bytes(value v_stat) {
  return caml_copy_int64(ocaml_eio_posix_stat_rdev_native(v_stat));
}

int64_t ocaml_eio_posix_stat_dev_native(value v_stat) {
  struct stat *s = Stat_val(v_stat);
  return s->st_dev;
}

value ocaml_eio_posix_stat_dev_bytes(value v_stat) {
...
CAMLprim value caml_eio_posix_unlinkat(value v_fd, value v_path, value v_dir) {
  ...
  caml_stat_free_preserving_errno(path);
  if (ret == -1)
    uerror("unlinkat", v_path);
  CAMLreturn(Val_unit);
}

CAMLprim value caml_eio_posix_renameat(value v_old_fd, value v_old_path,
                                       value v_new_fd, value v_new_path) {
  ...
  caml_enter_blocking_section();
  ret = renameat(Int_val(v_old_fd), old_path, Int_val(v_new_fd), new_path);
  caml_leave_blocking_section();
...
CAMLprim value caml_eio_posix_symlinkat(value v_old_path, value v_new_fd,
                                        value v_new_path) {
  CAMLparam2(v_old_path, v_new_path);
  size_t old_path_len = caml_string_length(v_old_path);
  size_t new_path_len = caml_string_length(v_new_path);
...
CAMLprim value caml_eio_posix_send_msg(value v_fd, value v_n_fds, value v_fds,
                                       value v_dst_opt, value v_bufs) {
...
static value safe_caml_unix_alloc_sockaddr(union sock_addr_union *adr,
...
CAMLprim value caml_eio_posix_recv_msg(value v_fd, value v_max_fds,
                                       value v_bufs) {
  CAMLparam1(v_bufs);
