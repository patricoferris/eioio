...
let await fut =
  ...
  Fiber_context.set_cancel_fn k.fiber (fun exn -> cancelled := true; let Suspend = Suspended.discontinue k exn in ());
  Fut.await fut (fun v ->
      Brr.Console.log [ Jstr.v "resolved and running"; !cancelled ];
      if not !cancelled then begin
        Fiber_context.clear_cancel_fn k.fiber;
        let Suspend = Suspended.continue k v in
        ()
      end
    );
  Scheduler.next st
...
let next_event : 'a Brr.Ev.type' -> Brr.Ev.target -> 'a Brr.Ev.t = fun typ target ->
  ...
  let v = listen (fun v -> Fiber_context.clear_cancel_fn k.fiber;
     let Suspend = Suspended.continue k v in
        ()) in
  Fiber_context.set_cancel_fn k.fiber (fun exn -> Ev.unlisten v; let Suspend = Suspended.discontinue k exn in
  ());
  Scheduler.next st
